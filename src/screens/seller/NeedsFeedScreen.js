...
export default function NeedsFeedScreen({ navigation }) {
  ...
  // Load current user and needs when screen mounts
  useEffect(() => {
    loadCurrentUser();
  }, []);
  ...
  const loadNeeds = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Extract unique categories from filtered needs
        const uniqueCategories = ['All', ...new Set(
          filteredNeeds.map(need => need.category)
        )];
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyTitle}>
            {selectedCategory === 'All' 
              ? 'No needs available yet' 
              : `No needs in ${selectedCategory}`}
          </Text>
          <Text style={styles.emptyText}>
            {selectedCategory === 'All'
              ? 'Check back soon for new opportunities from buyers'
